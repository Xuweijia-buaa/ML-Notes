...
一阶score: 离散特征，xi是one-hot/数值化后的。Field的每个位置作为新的特征，对应wi。 连续特征每个特征对应一个wi

二阶score:离散特征：尽管离散特征维护的embedding数目仍是one-hot后的，相当于一阶中每个one-hot后的特征对应一个embedding。

​                                    但每个样本仍根据离散特征的取值，映射到对应的embedding上。所以计算交互时，每个样本仍然只对应原始Field个离散embedding,

​                                   对应的xi是1。 而每个连续特征对应一个embedding，乘上对应的xi后。和所有原始离散Field对应的embedding两两内积求和（可简化）                      
